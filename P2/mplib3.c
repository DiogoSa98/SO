...
typedef struct message_t {
  ...
  int       consumed;
	...
	/* Structure with mutex and condition variables */
...
|              Em caso de erro devolve NULL. Cria tambem um mutex 
* 			   para cada canal.
---------------------------------------------------------------------*/

Channel_t* createChannel () {
  
	/* Allocates memory for channel */
  ...
  if (&channel->exMutex == NULL) {
      free(channel);
      return NULL;
  }
  
	/* Check error */
  if(pthread_mutex_init(&channel->exMutex, NULL) != 0) {  //           
    fprintf(stderr, "\nErro ao inicializar mutex\n");
    return NULL;
  }
  
  if (&channel->wait_for_free_space == NULL) {  
      free(channel);
      return NULL;
  }
  ...
  if (&channel->wait_for_messages == NULL) {
      free(channel);
      return NULL;
  }
  ...
  for (i=0; i<number_of_tasks; i++) {
    for (j=0; j<number_of_tasks; j++) {
      ...
      Message_t   *mess    = (Message_t*) leQueRemFirst(channel->message_list);
        ...
        if(pthread_mutex_destroy(&channel->exMutex) != 0) {          
...
int receberMensagem(int tarefaOrig, int tarefaDest, void *buffer, int tamanho) {
  ...
  channel = (Channel_t*) channel_array[tarefaDest*number_of_tasks+tarefaOrig];
	...
	/* Lock the exclusive Mutex for channel */
  if(pthread_mutex_lock(&channel->exMutex) != 0) {
  ...
  while (!mess) {
    if(pthread_cond_wait(&channel->wait_for_messages, &channel->exMutex) != 0) {
  ...
  if(pthread_mutex_unlock(&channel->exMutex) != 0) {
...
int enviarMensagem(int tarefaOrig, int tarefaDest, void *msg, int tamanho) {
  ...
  if (channel_capacity >0) {
    while (leQueSize(channel->message_list) >= channel_capacity) {
      if(pthread_cond_wait(&channel->wait_for_free_space, &channel->exMutex) != 0) {
        fprintf(stderr, "\nErro ao esperar pela variável de condição\n");
        return -1;
      }
    }
